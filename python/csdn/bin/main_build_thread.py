...
# 列表数据 key
redis_list_key = 'python:csdn:list:list'
# 待存数据库列表key
redis_cont_key = 'python:csdn:list:cont'

# dha.redis.delete(redis_list_key,'hyperloglog:csdn:itemid:',redis_cont_key)
# ---------------------------
...
def thread_cont_build():
    ...
    url = dha.get_content_url(items['user_name'],items['item_id']) #内容url
