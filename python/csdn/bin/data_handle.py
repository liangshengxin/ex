...
class DataHandle:
    ...
    # 内容页url生成
    def get_content_url(self,user_name,item_id):
        return self.__config_climb['host']+str(user_name)+'/article/details/'+str(item_id)
    ...
    def __findData(self,content):
        ...
        item = str(soup.find(class_="baidu_pl"))
        if not item:
            return False
        return item
    ...
    def __listData(self,lists):
        ...
        for item in lists:
            ...
            url_list[field['item_id']] = self.get_content_url(field['user_name'],field['item_id'])
